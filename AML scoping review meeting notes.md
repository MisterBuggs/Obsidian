...
The number of possible searches from such a table without allowing a completely empty search string is 2<sup>(k<sub>1</sub>+k<sub>2</sub>[...]+k<sub>i</sub>)</sup>-1, where i is the number of columns and k is the number of items per column; for the table above, that would be
